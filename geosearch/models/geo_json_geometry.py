...
class GeoJSONGeometry(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="`MultiPolygon` for most areas, `Polygon` for areas with a single ring — including areas that BECOME single-ring under `?simplify=`. Do not pin this to one value.")
    coordinates: List[Any] = Field(description="Nesting depth depends on `type`: three levels for `Polygon`, four for `MultiPolygon`. Positions are `[longitude, latitude]` in EPSG:4326, per the GeoJSON specification.")
