...
class QuotaDetail(BaseModel):
    ...
    """ # noqa: E501
    limit: StrictInt = Field(description="Monthly request allowance for the account's current plan.")
    used: StrictInt = Field(description="Requests consumed in the current quota period.")
    resets_at: datetime = Field(description="Start of the next quota period, when `used` returns to zero. Matches the `X-RateLimit-Reset` header on the same response, expressed as RFC 3339 rather than an epoch second.")
    ...
    __properties: ClassVar[List[str]] = ["limit", "used", "resets_at", "upgrade_url"]
