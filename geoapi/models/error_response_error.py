...
class ErrorResponseError(BaseModel):
    ...
    code: StrictStr
    message: StrictStr
    details: Optional[List[ErrorResponseErrorDetailsInner]] = None
    request_id: StrictStr = Field(description="Correlation identifier present on every error response. Quote this when contacting support.")
    trace_id: Optional[StrictStr] = Field(default=None, description="W3C trace ID of the distributed trace for this request, when tracing is enabled. Omitted entirely when no span was recording, so clients must treat it as optional. It complements rather than replaces `request_id`.")
    quota: Optional[QuotaDetail] = None
    upgrade: Optional[UpgradeDetail] = None
    __properties: ClassVar[List[str]] = ["code", "message", "details", "request_id", "trace_id", "quota", "upgrade"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "quota": QuotaDetail.from_dict(obj["quota"]) if obj.get("quota") is not None else None,
            "upgrade": UpgradeDetail.from_dict(obj["upgrade"]) if obj.get("upgrade") is not None else None
        })
        return _obj
